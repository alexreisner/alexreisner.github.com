...
Nicole's reason for this is consistency: if you copy an &lt;h2&gt; from one part of a page and paste it somewhere else, it should look the same. This is one of the rules of OO stylesheet design, the aim of which is to create code that is more modular, reusable, readable, and maintainable. There's also generally much less of it (smaller download).

There's another reason to follow this rule: "according to Google":http://code.google.com/speed/page-speed/docs/rendering.html#UseEfficientCSSSelectors, "inefficient" location-based selectors are rendered more slowly by web browsers (try Google's "PageSpeed":http://code.google.com/speed/page-speed plugin for Firebug to see how your selectors rate).
